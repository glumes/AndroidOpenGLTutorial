...
/**
 * Created by glumes on 20/07/2018
 */
class DepthTestFragment : Fragment() {

    lateinit var surfaceView: DepthTestSurfaceView

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        surfaceView = DepthTestSurfaceView(context!!)
...
class DepthTestSurfaceView(var mContext: Context) : GLSurfaceView(mContext) {

    init {
        setEGLContextClientVersion(2)
        setRenderer(DepthTestRenderer())
        renderMode = GLSurfaceView.RENDERMODE_CONTINUOUSLY
    }

    inner class DepthTestRenderer : GLSurfaceView.Renderer {
